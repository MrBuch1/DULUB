...
class ApiController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $situacaoCliente = json_decode($client->request('GET', 'https://visions.topmanager.com.br/Servidor_2.1.1_api/forcadevendas/contasareceber/consultar?vendedorID=' . Auth::id(),
            ['headers' => ['Content-Type' => 'application/json', 'Authorization' => 'Bearer ' . $token]])->getBody(), true);


        return view('API.form_1', compact('token', 'empresa', 'tipoOP', 'cliente', 'vendedor', 
                                            'situacaoCliente', 'vendedorUser'));
    }
    ...
    public function situacaoCliente(Request $request)
    {
        ...
        $situacaoCliente = json_decode($client->request('GET', 'https://visions.topmanager.com.br/Servidor_2.1.1_api/forcadevendas/contasareceber/consultar?vendedorID=' . $vendedorUser,
            ['headers' => ['Content-Type' => 'application/json', 'Authorization' => 'Bearer ' . $token]])->getBody(), true);

        return view('API.situacaoCliente', compact('situacaoCliente', 'vendedorUser', 'cliente',
                                                    'clienteID', 'empresaID', 'opID', 'faturamento'));
    }
    ...
    public function Unidade(Request $request)
    {
        ...
        return view('API.unidade', compact('client', 'token', 'cliente', 'clienteID', 'objetoID', 'objeto', 'unidade',
    ...
    {        
        ...
        return view('API.teste_pedido', compact('store', 'client', 'token', 'cliente', 'clienteID', 'objeto', 'objs',
                                                'qtds', 'body'));
