<html>
  ...
  <body>
  <form action="/json-api5" method="GET">
  @csrf
      ...
      <div class="card-body">
        ...
        <div class="form-group">
          ...
          <select class="selectpicker mb-3" name="formaPagamento" id="formaPagamento" data-live-search="true">
            @foreach($FormasPagamento as $fp1 => $fp2)
              <option data-tokens="{{ $fp1 }}" value="{{ $fp1 }}">{{ $fp2 }}</option>
            @endforeach
          </select>
        </div>
        ...
        <div class="form-group">
          ...
          <input id="obs" name="obs" type="text" placeholder="Observação">
        </div>

        <div class="form-group">
          <label for="comInterna"><strong>Comunicação Interna: </strong></label>
          <input id="comInterna" name="comInterna" type="text" placeholder="Comunicação Interna">
        </div>
      </div>
    </div>
